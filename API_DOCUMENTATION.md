...
```json
{
  ...
  "requirements": ["3+ years of experience", "React proficiency"],
...
**cURL Example:**
```bash
curl "http://localhost:3001/api/jobs?location=New York&type=Full-time&remote=true&limit=10"
```
...
**Query Parameters:**
- `q`: Search query (searches title, description, company)
- `location`: Filter by location
- `type`: Job type
...
**cURL Example:**
```bash
curl "http://localhost:3001/api/jobs/search?q=developer&location=New York&skills=JavaScript,React&salary_min=80000&limit=10"
```
...
**Description:** Create a new job posting on job board

**Authentication:** Required

**Request Body:** Same as `/api/jobs` endpoint
...
**Query Parameters:** Same as `/api/jobs` endpoint

**cURL Example:**
```bash
curl "http://localhost:3001/api/job-board/jobs?location=New York&type=Full-time"
```
...
**Query Parameters:** Same as `/api/jobs/search` endpoint

**cURL Example:**
```bash
curl "http://localhost:3001/api/job-board/jobs/search?q=developer&skills=JavaScript,React" \
  -H "Authorization: Bearer <your_token>"
```
...
**Request Body:** Same as `/api/jobs/:id` PUT endpoint
