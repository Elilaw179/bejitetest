...
const ProfileStats = ({ onConnect }) => (
  <div className="p-8">
    ...
    <div className="mt-4">
      <p className="text-[#E09A36]">Graphics Designer</p>
      <p className="text-[#6B8E23]"> @Lagos, Nigeria</p>
    </div>
    <StatsGrid />
...
const StatItem = ({ value, label }) => (
  <div className="flex gap-2">
    <p className="text-[#E09A36] font-semibold text-[16px]">{value}</p>
    <p className="text-[#556B1F] font-semibold text-[16px]">.{label}</p>
  </div>
);
...
const Button = ({ icon, text, onClick }) => (
  <button
    onClick={onClick}
    className="bg-[#556B1F] w-[250px] text-center text-[#FFFFFF] flex p-1.5 rounded-3xl gap-2 justify-center"
  >
...
const Divider = () => (
  <div className="max-w-3xl mx-auto my-8 border-t-2 border-[#16730F]"></div>
);

const ProfileSection = ({ title, children }) => (
  <div className="bg-[#556B1F] rounded-2xl mt-2">
    <div className="pt-8 px-8">
      <p className="text-[#E09A36] text-[24px] mb-1">{title}</p>
    </div>
    ...
    <div className="px-8 pb-8 rounded-2xl text-[#FFFFFF] space-y-4 items-center">
      {skills.map((skill, index) => (
        <div key={index}>
          ...
          <p className="text-[10px] font-medium">{skill.experience}</p>
        </div>
      ))}
    </div>
  );
    ...
    <div className="px-8 pb-8 rounded-2xl text-[#FFFFFF] space-y-4 items-center">
      {contacts.map((contact, index) => (
        <div key={index} className="flex gap-3">
          ...
          <div>
            <p className="text-[14px] font-semibold">{contact.value}</p>
            <p className="text-[10px] font-medium">{contact.type}</p>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="px-8 pb-8 rounded-2xl text-[#FFFFFF] space-y-4 items-center">
      <div className="flex gap-3">
        ...
        <div>
          ...
          <button
            onClick={onConnect}
            className="bg-[#1A3E32] w-40 flex items-center justify-center space-x-1 p-2 rounded-3xl">
