...
const AdminLayout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white shadow-lg transform transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between h-20 px-6 border-b border-gray-100">
          <img src="/assets/images/logo.png" alt="Bejite Logo" className="h-8" />
        ...
        <div className="px-4 py-6 flex flex-col h-[calc(100vh-5rem)] justify-between">
          <nav className="space-y-1">
            <p className="px-2 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Analytics</p>
            {navItems.map((item) => (
              <NavLink
                key={item.name}
                to={item.path}
                ...
                onClick={() => setSidebarOpen(false)}
              >
                <item.icon size={20} className={item.path === '/admin/dashboard' ? '' : 'text-gray-400'} />
