...
export default function StageBuilderModal({ isOpen, onClose, onAddStage }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (onAddStage) {
      const result = await onAddStage({
        ...
        interviewer: interviewer.trim() || "Technical Panel",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden p-5 sm:p-7 shadow-2xl relative border border-gray-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center gap-3.5 mb-4 shrink-0 pr-8">
          ...
          <div>
            <h3 className="text-[16px] sm:text-xl font-bold text-[#1A3E32] tracking-tight">
              Stage Builder
            </h3>
            <p className="text-xs text-gray-500 font-normal mt-0.5">
              Configure your hiring pipeline to track candidates from
              application to hire.
            </p>
          </div>
        </div>
          ...
          <div className="overflow-y-auto nfl-scroll pr-1 sm:pr-2 flex-1 min-h-0 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white border border-gray-300 text-gray-800 text-[12px] md:text-sm p-3.5 sm:p-4 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#16730F]/40 placeholder:text-gray-400 font-medium resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white border border-gray-300 text-gray-800 text-[12px] md:text-sm px-4 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#16730F]/40 placeholder:text-gray-400 font-medium"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-[#1A3E32] mb-1">
