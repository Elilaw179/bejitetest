        ...
        <div className="min-h-0">
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="bg-[#1A3E32] px-6 py-8 text-center">
              <h1 className="text-3xl font-bold text-white">Advanced Search Engine</h1>
              <p className="text-green-100 mt-2 text-lg">Find the perfect candidates for your hiring needs</p>
            </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
                      <svg className="w-5 h-5 text-[#16730F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="px-6 py-4 border-b bg-gray-50">
              <div className="flex items-center justify-center gap-4">
                <span className={`text-sm font-medium ${currency === "USD" ? "text-[#1A3E32]" : "text-gray-400"}`}>USD ($)</span>
                <button
                  onClick={() => setCurrency(currency === "USD" ? "NGN" : "USD")}
                  className="relative inline-flex h-6 w-11 items-center rounded-full bg-[#1A3E32] transition-colors"
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${currency === "NGN" ? "translate-x-6" : "translate-x-1"}`} />
                </button>
                <span className={`text-sm font-medium ${currency === "NGN" ? "text-[#1A3E32]" : "text-gray-400"}`}>NGN (₦)</span>
              </div>
            </div>

            {/* Plans */}
            <div className="p-6">
              <h2 className="text-xl font-bold text-center text-[#1A3E32] mb-6">Choose Your Plan</h2>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {plans.map((plan) => (
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <div className="mt-4">
                        <span className="text-4xl font-bold text-[#1A3E32]">
                          {currency === "USD" ? `$${plan.price}` : `₦${plan.priceNaira}`}
                        </span>
                        <span className="text-gray-500 text-sm">/{plan.billingPeriod || "search"}</span>
                      </div>
                      ...
                      >
                        {processing && selectedPlan?.id === plan.id ? "Processing..." : plan.type === "one_time" ? "Pay Now" : "Subscribe Now"}
                      </button>
                    </div>
                    ...
                    <ul className="mt-4 space-y-3">
                      {plan.features?.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-sm text-gray-600">
                          <svg className="w-5 h-5 text-[#16730F] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                          </svg>
                          {feature}
                        </li>
                    ...
                    >
                      {processing && selectedPlan?.id === plan.id ? "Processing..." : plan.type === "one_time" ? "Pay Now" : "Subscribe"}
                    </button>
                  </div>
                ))}
              </div>
            ...
            <div className="px-6 pb-6">
              <h3 className="text-lg font-bold text-[#1A3E32] mb-4 text-center">Plan Comparison</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b-2 border-[#1A3E32]">
                      <th className="py-3 text-left text-gray-600">Feature</th>
                      <th className="py-3 text-center text-[#1A3E32] font-bold">Standard</th>
                      <th className="py-3 text-center text-[#16730F] font-bold">Premium</th>
                      <th className="py-3 text-center text-[#1A3E32] font-bold">Jumbo</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="border-b">
                      <td className="py-3 text-gray-600">Candidates per search</td>
                      <td className="py-3 text-center font-semibold">20</td>
                      <td className="py-3 text-center font-semibold">20</td>
                      <td className="py-3 text-center font-semibold text-[#16730F]">30</td>
                    </tr>
                    <tr className="border-b">
                    ...
                    <tr className="border-b">
                      <td className="py-3 text-gray-600">Save card for future</td>
        ...
        <div className="hidden md:block">
          <div className="bg-white rounded-lg shadow p-4 sticky top-20">
            ...
            <div className="space-y-3 text-sm text-gray-600">
              <p>Contact our support team for assistance with choosing the right plan.</p>
              <button 
