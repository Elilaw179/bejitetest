...
export async function verifyLoginTwoFactorRequest({ twoFactorToken, challengeId, code }) {
  const response = await axiosPublic.post("/auth/login/2fa", {
    twoFactorToken,
    challengeId,
    code,
  });
...
export async function requestTwoFactorRecovery({ twoFactorToken, challengeId }) {
  try {
    const { data } = await axiosPublic.post("/auth/2fa/recovery/request", {
      twoFactorToken,
      challengeId,
    });
    return data;
  } catch (error) {
    throw new Error(readError(error, "Could not send recovery email"));
  }
}

export async function confirmTwoFactorRecovery(token) {
  try {
    const { data } = await axiosPublic.post("/auth/2fa/recovery/confirm", { token });
    return data;
  } catch (error) {
    throw new Error(readError(error, "Could not complete 2FA recovery"));
  }
}
